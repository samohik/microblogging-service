...
async def test_db(async_db):
    user = User(name="Test", email="678@gmail.com", hashed_password="4567")
    async_db.add(user)
...
class TestFollow:
    async def test_get_follower_exist(self, async_db):
        result = await Follow.get_follow(session=async_db, from_user=1, to_user=2)
        assert result == {"from": 1, "to": 2}

    async def test_get_follower_dont_exist(self, async_db):
        result = await Follow.get_follow(session=async_db, from_user=1, to_user=4)
        assert result == {}

    async def test_get_follower(self, async_db):
        data = await Follow.get_follower(session=async_db, id=1)
        assert data == [{"id": 3, "name": "Alt"}]

    async def test_get_following(self, async_db):
        data = await Follow.get_following(session=async_db, id=1)
